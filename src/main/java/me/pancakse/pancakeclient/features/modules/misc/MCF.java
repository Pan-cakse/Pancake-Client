package me.pancakse.pancakeclient.features.modules.misc;
    ...
    @Override public void onTick() {
        if (GLFW.glfwGetMouseButton(mc.getWindow().getHandle(), 2) == 1) {
            if (!pressed) {
                ...
                if (PancakeClient.friendManager.isFriend(name)) {
                    PancakeClient.friendManager.removeFriend(name);
                    Command.sendMessage(Formatting.RED + name + Formatting.RED + " has been unfriended.");
                } else {
                    PancakeClient.friendManager.addFriend(name);
