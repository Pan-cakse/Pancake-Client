package me.pancakse.pancakeclient.features.gui;
...
import me.pancakse.pancakeclient.features.gui.items.buttons.ModuleButton;
...
public class PancakeClientGui extends Screen {
    private static PancakeClientGui pancakeclientGui;
    private static PancakeClientGui INSTANCE;

    static {
        INSTANCE = new PancakeClientGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public PancakeClientGui() {
        super(Text.literal("PancakeClient"));
        setInstance();
        load();
    }

    public static PancakeClientGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new PancakeClientGui();
        }
        return INSTANCE;
    ...
    public static PancakeClientGui getClickGui() {
        return PancakeClientGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : PancakeClient.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    PancakeClient.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
